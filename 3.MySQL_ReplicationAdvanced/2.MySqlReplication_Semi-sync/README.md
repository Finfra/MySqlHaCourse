...
*semisynchronous replication을 적용하고 싶으면*
```
...
## 7.5 Semi-sync Test
* Example data install
```
ls empdept.sql
docker cp empdept.sql master:/root
docker exec -it master /usr/bin/mysql -uroot -pmypass -e "source /root/empdept.sql"
```
* master1 접근해서 데이터를 확장
```
docker exec -it master /usr/bin/mysql -uroot -pmypass
  use mydb
  create table e as select * from emp;
  insert into e select * from e;   -- 여러번 1000만개 넘을때 까지.
